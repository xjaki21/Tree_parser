...
class bag{
    ...
    public:
        ...
        bool empty() const;
        Val& next_to(Val& v);
        const Val& next_to(const Val& v) const;
        ...
        const_bag_iterator tail_it() const;
...
template <typename Val>
const Val& bag<Val>::next_to(const Val& v) const{
    pCell pc=head;
    bool found=false;
    while(pc!=nullptr && !found){
        if(pc->val==v){
...
template <typename Val>
typename bag<Val>::bag_iterator bag<Val>::tail_it(){
    return {tail};
}
...
template <typename Val>
typename bag<Val>::const_bag_iterator bag<Val>::tail_it() const{
    return {tail};
}
